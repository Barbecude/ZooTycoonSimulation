...
public class StaffEntity extends PathfinderMob {
    ...
    private static final java.util.Set<BlockPos> CLAIMED_TRASH = java.util.Collections
            .synchronizedSet(new java.util.HashSet<>());

    // Temporary placed blocks to help staff navigate around obstacles
    private final java.util.Map<Long, Integer> tempPlacedBlocks = new java.util.HashMap<>();
    ...
    public void setRole(int role) {
        ...
        if ("Staff".equals(getStaffName()) || getStaffName().isEmpty()) {
            this.setStaffName(
                    role == 1 ? randomZookeeperName() : (role == 2 ? randomSecurityName() : randomJanitorName()));
        }
    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType reason,
        ...
        // Jika spawn alami/telur, acak role. Jika command, biasanya role di-set setelah
        // spawn.
        ...
        if (getStaffName().isEmpty() || "Staff".equals(getStaffName())) {
            this.setStaffName(getRole() == 1 ? randomZookeeperName()
                    : (getRole() == 2 ? randomSecurityName() : randomJanitorName()));
        }
    ...
    protected void registerGoals() {
        ...
        // Role 1 (Zookeeper) - PRIORITIZE HUNGRY ANIMALS FIRST
        this.goalSelector.addGoal(2, new ThrowFoodToHungryAnimalGoal(this)); // Higher priority for hungry animals
        this.goalSelector.addGoal(3, new InteractVisitorGoal(this));
        this.goalSelector.addGoal(4, new RefillFeederGoal(this));
        this.goalSelector.addGoal(5, new PatrolSpecializedAnimalGoal(this)); // Patrol near specialist animal when idle
        ...
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this,
                net.minecraft.world.entity.monster.Monster.class, 10, true, false, null) {
            @Override
            public boolean canUse() {
                return super.canUse() && StaffEntity.this.getRole() == 2;
            }
        ...
        this.targetSelector.addGoal(2,
                new NearestAttackableTargetGoal<>(this, VisitorEntity.class, 10, true, false, (target) -> {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide) {
            lifeTicks++;
            // INCREASED FREQUENCY: Check specialization every 20 ticks (1 second) instead
            // of every 40
            if (getRole() == 1 && this.tickCount % 10 == 0) {
            ...
            // Non-permanent staff (like spawned security) leave after 5 minutes (6000
            // ticks)
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        if (gatePos != null)
            tag.putLong("GatePos", gatePos.asLong());
    ...
    private void ensureZookeeperSpecialization() {
        if (getRole() != 1 || this.level().isClientSide)
            return;
        // Keep current specialization if the animal still exists anywhere in ZooData
        if (!getAssignedAnimalId().isEmpty()) {
            ...
            for (int i = 0; i < tagged.size(); i++) {
                ...
                if (getAssignedAnimalId().equals(t.getString("type"))) {
                    net.minecraft.world.entity.Entity e = ZooAnimalHungerSystem.resolveTaggedEntity(
                            (net.minecraft.server.level.ServerLevel) this.level(), t);
                    if (e != null && e.isAlive()) {
                        // Still valid – ensure food is set
                        if (getAssignedFoodId().isEmpty()) {
                            List<net.minecraft.world.item.Item> foods = FoodAnimalRegistry
                                    .getFoodsForAnimal(getAssignedAnimalId());
                            if (!foods.isEmpty()) {
                                net.minecraft.resources.ResourceLocation foodId = net.minecraftforge.registries.ForgeRegistries.ITEMS
                                        .getKey(foods.get(0));
                                if (foodId != null)
                                    setAssignedFoodId(foodId.toString());
                            }
                        }
                        return; // keep specialization
                    }
                }
            }
            // Animal gone – reset
        ...
        // Need to pick a new specialization from ALL tagged animals globally
        if (!getAssignedAnimalId().isEmpty())
            return;
        ...
        // Collect already-assigned animal types from ALL zookeepers in the world
        java.util.Map<String, Integer> assignedCount = new java.util.HashMap<>();
        for (StaffEntity other : this.level().getEntitiesOfClass(StaffEntity.class,
                this.getBoundingBox().inflate(512.0D))) { // wide enough for entire zoo
            if (other == this || other.getRole() != 1)
                continue;
            if (!other.getAssignedAnimalId().isEmpty()) {
                assignedCount.merge(other.getAssignedAnimalId(), 1, Integer::sum);
            }
        }

        // Pick the animal type with fewest assigned keepers and existing food
        String best = "";
        ...
        for (int i = 0; i < tagged.size(); i++) {
            ...
            if (type == null || type.isEmpty())
                continue;
            if (FoodAnimalRegistry.getFoodsForAnimal(type).isEmpty())
                continue;
            int cnt = assignedCount.getOrDefault(type, 0);
            if (cnt < bestAssigned) {
                bestAssigned = cnt;
                best = type;
            }
        ...
        if (best.isEmpty())
            return;
        setAssignedAnimalId(best);
        List<net.minecraft.world.item.Item> foods = FoodAnimalRegistry.getFoodsForAnimal(best);
        if (!foods.isEmpty()) {
            net.minecraft.resources.ResourceLocation foodId = net.minecraftforge.registries.ForgeRegistries.ITEMS
                    .getKey(foods.get(0));
            if (foodId != null)
        ...
        org.apache.logging.log4j.LogManager.getLogger(IndoZooTycoon.MODID)
                .info("Zookeeper {} auto-specialised → {}", this.getId(), best);
    }
    ...
    static class RideVehicleGoal extends Goal {
        ...
        public boolean canUse() {
            if (staff.getRole() != 0)
                return false;
            if (staff.isPassenger() || staff.getRandom().nextFloat() > 0.02F)
                return false;
    ...
    static class CleanTrashGoal extends Goal {
        ...
        @Override
        public boolean canUse() {
            if (staff.getRole() != 0 || staff.getRandom().nextFloat() > 0.05F)
                return false;
        ...
        public void stop() {
            if (targetPos != null)
                CLAIMED_TRASH.remove(targetPos); // Release claim
            staff.setAnimState(0);
            targetPos = null;
        }
        ...
        public void tick() {
            ...
            if (staff.distanceToSqr(targetPos.getX(), targetPos.getY(), targetPos.getZ()) < 4.0D) {
                ...
                // Try to place blocks if stuck
                if (placeBlockCooldown > 0) {
                    placeBlockCooldown--;
                } else if ((staff.getNavigation().isStuck() || staff.horizontalCollision)
                        && staff.tempPlacedBlocks.size() < 12) {
                    if (staff.attemptClimbPlacement()) {
    ...
    static class RefillFeederGoal extends Goal {
        ...
        public boolean canUse() {
            if (staff.getRole() != 1 || staff.getRandom().nextFloat() > 0.05F)
                return false;
            if (staff.getAssignedAnimalId().isEmpty() || staff.getAssignedFoodId().isEmpty())
                return false;
            net.minecraft.world.entity.Entity specializedAnimal = findSpecializedAnimal();
            if (specializedAnimal == null)
                return false;
            net.minecraft.resources.ResourceLocation foodId = net.minecraft.resources.ResourceLocation
                    .tryParse(staff.getAssignedFoodId());
            net.minecraft.world.item.Item foodItem = foodId != null
                    ? net.minecraftforge.registries.ForgeRegistries.ITEMS.getValue(foodId)
                    : null;
            if (foodItem == null)
                return false;
            BlockPos p = staff.blockPosition();
            for (BlockPos pos : BlockPos.betweenClosed(p.offset(-15, -3, -15), p.offset(15, 3, 15))) {
                if (staff.level().getBlockState(pos).is(IndoZooTycoon.ANIMAL_FEEDER_BLOCK.get())) {
                    ...
                    if (!(be instanceof AnimalFeederBlockEntity feeder) || feeder.isFull())
                        continue;
                    if (specializedAnimal.blockPosition()
                            .distSqr(pos) > (SPECIALIZATION_RADIUS * SPECIALIZATION_RADIUS))
                        continue;
                    if (!feeder.canAcceptFood(foodItem, "", 1))
                        continue;
        ...
        public void start() {
            ...
            net.minecraft.world.item.Item foodItem = foodId != null
                    ...
                    : null;
            if (foodItem == null)
                foodItem = net.minecraft.world.item.Items.HAY_BLOCK;
            staff.setItemInHand(net.minecraft.world.InteractionHand.MAIN_HAND,
                    new net.minecraft.world.item.ItemStack(foodItem));
        }
                ...
                if (workTimer > 40) {
                    ...
                    if (be instanceof AnimalFeederBlockEntity feeder) {
                        ...
                        net.minecraft.world.item.Item foodItem = foodId != null
                                ...
                                : null;
                        if (foodItem != null
                                && FoodAnimalRegistry.isValidFoodForAnimal(staff.getAssignedAnimalId(), foodItem)) {
        ...
        private net.minecraft.world.entity.Entity findSpecializedAnimal() {
            ...
            for (int i = 0; i < tagged.size(); i++) {
                net.minecraft.nbt.CompoundTag t = tagged.getCompound(i);
                if (!staff.getAssignedAnimalId().equals(t.getString("type")))
                    continue;
    ...
    static class ThrowFoodToHungryAnimalGoal extends Goal {
        ...
        public boolean canUse() {
            ...
            if (staff.getAssignedAnimalId().isEmpty() || staff.getAssignedFoodId().isEmpty())
                return false;
            if (!(staff.level() instanceof net.minecraft.server.level.ServerLevel serverLevel))
                return false;
        ...
        public void start() {
            ...
            // Start moving toward animal immediately
            if (targetAnimal != null) {
                staff.getNavigation().moveTo(targetAnimal, 1.1D);
            }
        }
        ...
        public void tick() {
            ...
            // If farther than 8 blocks (64 = 8^2), move closer
            if (dist > 64.0D) {
                // Try to place blocks if stuck
                if (placeBlockCooldown > 0) {
                    placeBlockCooldown--;
                ...
                actionTicks = 0; // Reset throw timer while approaching
                return;
            }

            // Within throw range – stop and count down to throw
            staff.getNavigation().stop();
            actionTicks++;
            if (actionTicks >= 16) {
        ...
        /**
         * Search globally for a hungry animal of the assigned type.
         * No distance limit so the keeper will walk across the whole zoo if needed.
         */
        private LivingEntity findHungrySpecializedAnimal(net.minecraft.server.level.ServerLevel level) {
            ...
            for (int i = 0; i < tagged.size(); i++) {
                ...
                if (!staff.getAssignedAnimalId().equals(tag.getString("type")))
                    continue;

                net.minecraft.world.entity.Entity entity = ZooAnimalHungerSystem.resolveTaggedEntity(level, tag);
                if (!(entity instanceof LivingEntity living) || !living.isAlive())
                    continue;
                if (!ZooAnimalHungerSystem.isHungry(level, living))
                    continue;

                // No distance cap – keeper will travel to any hungry specialist
    ...
    /**
     * Makes idle zookeepers patrol near their specialist animal so they are
     * already close-by when the animal becomes hungry.
     */
    static class PatrolSpecializedAnimalGoal extends Goal {
        private final StaffEntity staff;
        private net.minecraft.world.entity.Entity targetAnimal;
        private static final double PATROL_DIST_SQ = 10.0D * 10.0D; // Stay within 10 blocks

        PatrolSpecializedAnimalGoal(StaffEntity staff) {
            this.staff = staff;
            this.setFlags(java.util.EnumSet.of(Goal.Flag.MOVE));
        }
        ...
        public boolean canUse() {
            ...
            if (staff.getAssignedAnimalId().isEmpty())
                return false;
            if (staff.getRandom().nextFloat() > 0.05F) // small throttle only
                return false;
            if (!(staff.level() instanceof net.minecraft.server.level.ServerLevel sLevel))
                return false;
            targetAnimal = findSpecialist(sLevel);
            if (targetAnimal == null)
                return false;
            // Start patrol if more than 20 blocks away from specialist
            return staff.distanceToSqr(targetAnimal) > (20.0D * 20.0D);
        }

        @Override
        public void start() {
            if (targetAnimal != null) {
                staff.getNavigation().moveTo(targetAnimal, 0.9D);
            }
        }
        ...
        public boolean canContinueToUse() {
            if (targetAnimal == null || !targetAnimal.isAlive())
                return false;
            if (staff.distanceToSqr(targetAnimal) <= PATROL_DIST_SQ)
                return false;
            return !staff.getNavigation().isDone();
        }

        @Override
        public void tick() {
            if (targetAnimal != null && staff.getNavigation().isDone()) {
                staff.getNavigation().moveTo(targetAnimal, 0.9D);
            }
        }

        private net.minecraft.world.entity.Entity findSpecialist(net.minecraft.server.level.ServerLevel level) {
            ZooData data = ZooData.get(level);
            net.minecraft.nbt.ListTag tagged = data.getTaggedAnimals();
            for (int i = 0; i < tagged.size(); i++) {
                net.minecraft.nbt.CompoundTag t = tagged.getCompound(i);
                if (!staff.getAssignedAnimalId().equals(t.getString("type")))
                    continue;
                net.minecraft.world.entity.Entity e = level.getEntity(t.getInt("id"));
                if (e != null && e.isAlive())
                    return e;
            }
            return null;
    ...
    static class InteractVisitorGoal extends Goal {
        ...
        public boolean canUse() {
            ...
            if (staff.getRandom().nextFloat() < 0.01F) {
                VisitorEntity nearby = staff.level().getNearestEntity(VisitorEntity.class,
                        TargetingConditions.forNonCombat().range(4.0D).selector(e -> e.hasLineOfSight(staff)), staff,
                        staff.getX(), staff.getY(), staff.getZ(), staff.getBoundingBox().inflate(4.0D));
                if (nearby != null) {
        ...
        public void start() {
            timer = 60;
            staff.getNavigation().stop();
            staff.setAnimState(staff.getRole() == 0 ? 2 : 3);
        }
        ...
        public boolean canContinueToUse() {
            return timer > 0 && targetVisitor != null && targetVisitor.isAlive()
                    && staff.distanceToSqr(targetVisitor) < 25.0D;
        }
    }
    ...
    // BLOCK PLACEMENT HELPERS FOR NAVIGATION
    private void tickTempBlocks() {
        ...
        while (it.hasNext()) {
            ...
            if (this.level().getBlockState(pos).is(net.minecraft.world.level.block.Blocks.DIRT)) {
                if (this.level().getEntities(null, new net.minecraft.world.phys.AABB(pos)).isEmpty()) {
    ...
    protected boolean attemptClimbPlacement() {
        if (tempPlacedBlocks.size() >= 12)
            return false;
        BlockPos feetPos = this.blockPosition();
        BlockPos placePos = null;

        // Try to place block at feet
        if (this.level().getBlockState(feetPos).isAir()) {
            placePos = feetPos;
        } else {
            // Try below feet
        ...
        try {
            this.level().setBlock(placePos, net.minecraft.world.level.block.Blocks.DIRT.defaultBlockState(), 3);
            tempPlacedBlocks.put(placePos.asLong(), 100);
